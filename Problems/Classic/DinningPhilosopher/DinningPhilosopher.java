package Problems.Classic.DinningPhilosopher;
...
public class DinningPhilosopher {
    ...
    private class PhilosopherRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                criticalSemaphore.acquire(); criticalAcquired = true;

                
                fork.get(left).acquire(); leftAcquired = true;
                System.out.println("Acquired left fork, with forkId: " + left + " for Philosopher: " + this.id);
                
                fork.get(right).acquire(); rightAcquired = true;
